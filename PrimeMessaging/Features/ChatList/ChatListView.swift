...
struct ChatListView: View {
    ...
    var body: some View {
        ...
        .task {
            await viewModel.loadChats(mode: mode, repository: environment.chatRepository)
        }
        .onChange(of: mode) { _, newValue in
            Task {
                await viewModel.loadChats(mode: newValue, repository: environment.chatRepository)
            }
        }
    }
}
...
final class ChatListViewModel: ObservableObject {
    ...
    @MainActor
    func loadChats(mode: ChatMode, repository: ChatRepository) async {
        do {
            chats = try await repository.fetchChats(mode: mode)
